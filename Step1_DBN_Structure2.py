...
def load_CodaLab_skel(ratio_train=0.9, ration_valid=0.1):
    ...
    #载入数据，一个数有动作的数据，一个是无动作的数据，都是字典类型
    f = file('Feature_train_realtime.pkl','rb' )#这个是有动作帧
    Feature_train = cPickle.load(f)
    f.close()

    f = file('Feature_all_neutral_realtime.pkl','rb' )#这是无动作帧
    Feature_train_neural = cPickle.load(f)
    f.close()

    #由于无动作帧太多，通过随机抽取部分，然后拼接到数据集all中
    ...
    Target_all_numeric = numpy.argmax(Target_all, axis=0)
    #效果：Target_all是2444行 201列矩阵，这里对列求最大，得到每一列

    #argmax是求最大值， 这里all是个矩阵，轴是从0开始的，（0  1  2 ==》 列 行 层==》y x z）
    #轴=1 ，就是对x轴上数据，一行求最大的序号.  是个tuple的结构，只有一个维度，
    #轴=0，就是对y轴上的数据， 一列求最大，  返回的也是一个tuple
    ...
    train_set_feature = Feature_all[0:train_end,:]#训练集合，只用了所有特征的80%
    train_set_new_target = Target_all_numeric[0:train_end]#这里返回是动作id的序号

    # Wudi added normalized data for GRBM
    [train_set_feature_normalized, Mean1, Std1]  = preprocessing.scale(train_set_feature)
    #numpy的方法，计算均值和方差， 归一化的结果
    ...
    #这里直接一个（） 就将数据打包为一个 tuple 了

    #将验证数据也归一化
    valid_set_feature = Feature_all[train_end:valid_end,:]
    valid_set_new_target = Target_all_numeric[train_end:valid_end]#这是从000100序列 数字序号话的结果
...
def test_GRBM_DBN(finetune_lr=1, pretraining_epochs=100,
    ...
    dbn = GRBM_DBN(numpy_rng=numpy_rng, n_ins=528,#最开始的特征是528维度
              hidden_layers_sizes=[2000, 2000, 1000],#三层
    ...
    for i in xrange(dbn.n_layers):      
        ...
        for epoch in xrange(pretraining_epochs):
            ...
            + ' ran for %d sec' % ((end_time_temp - start_time_temp) )
    ...
    ########################
    # FINETUNING THE MODEL            整体调优步骤 #
    ########################

    # get the training, validation and testing function for the model
    print '... getting the finetuning functions'
    #构建调优函数， 这里返回了三个 东西 第一个是训练函数，
    # model是 对测试数据的损失评估值
    ...
    while (epoch < training_epochs) and (not done_looping):
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            minibatch_avg_cost = train_fn(minibatch_index)#调优
            iter = (epoch - 1) * n_train_batches + minibatch_index

            if (iter + 1) % validation_frequency == 0:
                import warnings
                warnings.filterwarnings("ignore")
                validation_losses = validate_model()#验证
