...
#提取开始帧到结束帧的骨架坐标，保存到矩阵中，返回是否归0 中心化
...
def Extract_feature_Realtime(Pose, njoints):
    ...
    for joints1 in range(njoints-1):
        for joints2 in range(joints1+1,njoints):
            Fcc[:, FeatureNum*3:(FeatureNum+1)*3] = Pose[:, joints1*3:(joints1+1)*3]-Pose[:, joints2*3:(joints2+1)*3];
            FeatureNum += 1
    #这里是直接对1885个样本 操作，直接每一列就是某个骨架点所有帧坐标，做差值
    ...
    for joints1 in range(njoints):
        for joints2 in range(njoints):
            Fcp[:, FeatureNum*3: (FeatureNum+1)*3] = Pose[1:,joints1*3:(joints1+1)*3]-Pose[0:-1,joints2*3:(joints2+1)*3]
            FeatureNum += 1
    #第一个pose是1---最后行， 第二个是0--到最后，所以就是当前帧-前一帧了，这里是对所有帧的数据直接操作
    ...
    #python中的序号 -n表示不要最后n个， 【-2：-9】表示从0开始向左循环数，第二个。 到左循环数第9个
    return Features
...
def viterbi_path_log(prior, transmat, observ_likelihood):
    ...
    Wudi first implement the forward pass.
    Future works include forward-backward encoding
    input: prior probability 1*N... 隐含状态的概率
    transmat: N*N               隐含状态的转移概率矩阵
    observ_likelihood: N*T   可见状态的概率

    这个函数其实完完全全就是按照维比特算法实现的，只是用了log处理数据，那么加法成了 原始数据的乘积
    同时 HMM模型中，有观测状态，和隐藏状态。 这里只有隐藏的201个动作id。观测的状态是rbm的处理结果。
    通过逻辑层得到的一个概率数组是 （当前观测状态 是隐藏状态i的概率）：  P（是隐藏状态i |每个观测状态）
    """
    T = observ_likelihood.shape[-1]
    N = observ_likelihood.shape[0]
    ...
    # step1 初始化
    global_score[:, 0] = prior + observ_likelihood[:, 0]
    #observ_likelihood[:, 0] 是一个视频的第一帧 每个动作id的可能度。
    # need to  normalize the data
    
    #step2 归纳计算：
    for t in range(1, T):#t是帧
        for j in range(N):#j是动作id
            temp = global_score[:, t-1] + transmat[:, j] + observ_likelihood[j, t]
            global_score[j, t] = max(temp)#t时刻是j状态的概率
            predecessor_state_index[j, t] = temp.argmax()#使得 t时刻是j状态概率最大的  前一刻的序号？？
            #保存的是：  t时刻是状态j ，那么前一刻是i的序号，
            #为的就是最后路径的回溯

    #step3 终结:
    path[T-1] = global_score[:, T-1].argmax()#argmax返回的是序号， max返回最值。
    #最后时刻， 到底哪个状态概率最大。 得到动作id序号
    
    #step4 路径回溯:
    for t in range(T-2, -1, -1):
        path[t] = predecessor_state_index[ path[t+1], t+1]#回溯路径
...
def viterbi_endframe(path ,perframe=5,framelen=30):
    ...
    #每个完整动作都分10个小id。 那么动作的开始是前0 1 2 3 个id，结束是最后的6789   id。

    end_frame=-1
    pred_label = -1  #存大的动作判断值

    frame = path.shape[-1]-1#从最后第二帧开始
    tureframlen=0;
    '''这一段就是从path里，分离出每个大动作的开始和结尾、标签'''
    for i in range(perframe):
        if path[frame-i] in end_label and  path[frame-i+1]==200:#这是一个动作的结束
            j=0;
            while(j<framelen ):
                if( path[frame-i-j] /10 == path[frame-i]/10 ):
                    j -=1 
                    tureframlen += 1
                else :
                    break
            if tureframlen >= 15 :
                pred_label= path[frame-i]/10+1
            else :
                tureframlen =0
    return  pred_label
...
def viterbi_colab_clean(path, global_score, threshold=-3, mini_frame=15):
    ...
    while(frame< path.shape[-1]-1):
        if path[frame-1]==200 and path[frame] in start_label:
            ''' 如果前一帧正好是空动作id， 且是当前帧是一个大动作的开始id。
            那么记录开始帧，记录大动作到 label'''
            #path保存了每一帧最可能是哪个动作小id。
            #201个小动作id，是从0开始，200正好是那个空动作了。

            begin_frame.append(frame)
            # python integer divsion will do the floor for us :)
            pred_label .append( path[frame]/10 + 1)#id除以10+1就是大动作id了
            frame += 1
        elif path[frame] in end_label and path[frame+1]==200:
            ''' 如果当前帧是一个大动作的结束id， 且下一帧是空动作id。
            那么记录结束帧 '''
    ...
    #一个动作有开始有结束，这里是为了防止错误
    if len(begin_frame)> len(end_frame):
        begin_frame = begin_frame[:-1]
        ''' python的数组问题： a=[1,2,3,4] . 那么a【：-1】==》等价于a【：3】是去除了最后一个的，
        a【0：4】是左闭右开的集合。'''
    ...
    frame_length = end_frame - begin_frame#这里都是向量array
    ## now we delete the gesture outside the vocabulary by choosing
    ## frame number small than mini_frame
    mask = frame_length > mini_frame#动作小于15的都是false了

    #通过false掩码操作，去除少于15帧的动作判断
    begin_frame = begin_frame[mask]
    end_frame = end_frame[mask]
    pred_label = pred_label[mask]
    
    ''' 维特比算法中， 概率是不断乘积上去的，（使用ln后，就是线性增加的）
    每转一个状态，那么global_score 会小， 通过每一帧的平均概率降低值， 
    如果《阈值，那么就是错误的
    '''

    Individual_score = []
    for idx, g in enumerate(begin_frame):
            '''enumerate是获取一个数组的index 和值。'''
            score_start = global_score[path[g], g]#g帧时刻， 状态是path【g】的概率值
    ...
    Individual_score = numpy.array(Individual_score)#【】只是list，要转array才能向量计算。
